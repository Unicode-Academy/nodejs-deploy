...
export class PublicController {
  async paginatedIndex(req, res) {
    try {
      ...
      res.render(path.resolve(VIEWS, "public", "homepage"), {
        title: "Homepage 123",
        images: images,
        products: products,
  ...
  async goToProduct(req, res) {
    try {
      ...
      res.render(path.resolve(VIEWS, "public", "product", "product-details"), {
        title: "Product",
        product: product,
        images: images,
        user: user,
      ...
      res
        .status(404)
        .render(path.resolve(VIEWS, "404.ejs"), {
          title: "Error",
  ...
  async goToCart(req, res) {
    ...
    res.render(path.resolve(VIEWS, "public", "product", "cart.ejs"), {
      title: "Shopping cart",
      user: user,
      cart: cart,
      products: products,
  ...
  async removeFromCart(req, res) {
    try {
      ...
      let resultStock = await Product.updateStock(
        product_id,
  ...
  async updateCart(req, res) {
    try {
      ...
      let resultCart = await Product.updateInCart(
        customer_id,
        product_id,
        new_quantity,
      );
  ...
  async validateShipping(req, res) {
    ...
    if (!validated) {
      ...
      res.render(path.resolve(VIEWS, "public", "product", "checkout-1.ejs"), {
        title: "Checkout",
        address: address,
        zipCode: zipCode,
        country: country,
      ...
      const promise = User.addShippingDetails(req.user, [
        address,
        zipCode,
        country,
        phoneNumber,
  ...
  async validatePayment(req, res) {
    ...
    if (
      !regExVisa.test(ccNumberCurated) &&
      !regExMastercard.test(ccNumberCurated)
    ) {
      validated = false;
    ...
    if (!validated) {
      res.render(path.resolve(VIEWS, "public", "product", "checkout-2.ejs"), {
        title: "Checkout",
        ccNumber: ccNumber,
        cvvNumber: cvvNumber,
        message: error,
    ...
    createOrderPromise
      .then((orderId) => {
        products.forEach((product) => {
          let promise = Product.createOrderItem(
            orderId,
            product.id,
            product.price,
            product.quantity,
